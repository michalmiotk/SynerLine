...
Mat image_processing(void* void_pointer, Mat camera_frame){
		...
		*/ 
		...
		if(ilosc){
		  pwmb=wartosc_zadana-(f->kp*blad+f->kd*rozniczka);//+f->kkat*najwiekszy_kat/90);
		  pwma=wartosc_zadana+(f->kp*blad+f->kd*rozniczka);//+f->kkat*najwiekszy_kat/90);
		}else
		{
...
void* find_horizontala(void* argument)
{
	...
	for(int rows=1; rows<arg->mat_in.rows/2; rows++)
  {
	  for(int columns=1 ; columns<arg->mat_in.cols-5*line_length;columns++)
	  {
		...
		*/
		if( abs(srednia_lewa-srednia_prawa)<arg->plus_roznica_lewaprawa && (srednia_lewa-srednia_srodek>(prog_histogram+arg->plus_prog)) && (srednia_prawa-srednia_srodek>(prog_histogram+arg->plus_prog)))// && odchylenie_lewa+odchylenie_prawa<arg->odchylenie_standardowe*4000)
		{	
...
int pid_silnik_lewy(int predkosc_zadana)
{
		...
		if(wypelnienie>1000) wypelnienie = 1000;
