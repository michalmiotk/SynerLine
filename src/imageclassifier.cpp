...
int stop_licznik=0;
int h_min=82; // 90 100 40 110 255 255
int h_max=104;
int s_min=99;
int s_max=163;
int v_min=40;
...
int main(int argc, char** argv){
	 ...
	 cvCreateTrackbar("h_min", "circle", &h_min, 255 ,NULL);
	 cvCreateTrackbar("h_max", "circle", &h_max, 255 ,NULL);
	 cvCreateTrackbar("s_min", "circle", &s_min, 255 ,NULL);
	 cvCreateTrackbar("s_max", "circle", &s_max, 255 ,NULL);
	 cvCreateTrackbar("v_min", "circle", &v_min, 255 ,NULL);
	 cvCreateTrackbar("v_max", "circle", &v_max, 255 ,NULL);
	 ...
	 for(int i=0;i<1000000;i++) {
			...
			for(int c=0;c<2;c++)erode(stop_range,stop_range,element2);
			//for(int c=0;c<2;c++)dilate(stop_range,stop_range,element2);
			...
			{ 
 			...
 			if(maxr_stop)
 			{
				...
				//resize(stop_split[2], stop_split[2], Size(140,140));
				imshow("wyciety stop",stop_split[2]);
			...
			//imshow("kolorowy",kolorowy);
			vector<Vec3f> circles;
			//cout<<"mieszanie"<<endl;
			//Scalar lower[3]={90,0,0};
			//Scalar upper[3]={110,255,255}; 90 100 40 110 255 255
			inRange(niebieski, Scalar(h_min,s_min,v_min),Scalar(h_max,s_max,v_max), niebieski_range);	
			Mat element=getStructuringElement(MORPH_RECT, Size(3,3));
			for(int c=0;c<6;c++)dilate(niebieski_range,niebieski_range,element);
			for(int c=0;c<1;c++)erode(niebieski_range,niebieski_range,element);
			if(pokaz_capture){imshow("niebieski", niebieski_range); imshow("czerwony",stop_range);};
...
void* test_stop(void* argument)
{
	...
	vector<Rect> stops2;   
	klasyfikator_stop.detectMultiScale(((struct strukturadlawatku*)argument)->obraz, stops2, 1.01, 3, 0|CV_HAAR_SCALE_IMAGE, Size(30, 30) );  
	for(unsigned k = 0 ; k<stops2.size(); k++)
	{
			stop_licznik++;
			cout<<"stop30"<<endl;
	}
	* */
	vector<Rect> stops3;   
	klasyfikator_stop.detectMultiScale(((struct strukturadlawatku*)argument)->obraz, stops3, 1.01, 3, 0|CV_HAAR_SCALE_IMAGE, Size(20, 20) );  
	for(unsigned k = 0 ; k<stops3.size(); k++)
	{
			stop_licznik++;
			cout<<"stop40"<<endl;
	}
	vector<Rect> stops4;   
	klasyfikator_stop.detectMultiScale(((struct strukturadlawatku*)argument)->obraz, stops4, 1.01, 3, 0|CV_HAAR_SCALE_IMAGE, Size(50, 50) );  
	for(unsigned k = 0 ; k<stops4.size(); k++)
	{
			stop_licznik++;
			cout<<"stop50"<<endl;
	}
...
int find_contour(Mat image2,int r, int x, int y)
{
    ...
    int ilosc_pikseli_x=35; //uwazac zeby nie przepelnic zmiennej
	int ilosc_pikseli_y=15;
	int lewo=0, prawo=0;
	int sumal=0;
	int sumap=0;
	//imshow("niebieski_range",niebieski_range);
	...
	for(int i=0; i<ilosc_pikseli_x;i++){
		for(int j=0;j<ilosc_pikseli_y;j++){
			int wx=x-r/2+i-ilosc_pikseli_x/2;
			int wy=y+r/2+j-ilosc_pikseli_y/2;
	...
	//imshow("linie",kur);
	if(sumal<sumap){lewo=1; prawo=0; cout<<"L"<<endl;} if(sumal>sumap){lewo=0; prawo=1;cout<<"P"<<endl;};
    findContours(imageresult1,contours1,hierarchy1,CV_RETR_CCOMP,CV_CHAIN_APPROX_SIMPLE,cvPoint(0,0));
    /*
	...
	*/
    findContours(imageresult2,contours2,hierarchy2,CV_RETR_CCOMP,CV_CHAIN_APPROX_SIMPLE,cvPoint(0,0));
    /*
