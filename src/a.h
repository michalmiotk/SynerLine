...
void create_windows_trackbars(struct text* a, int* min_canny, int* max_canny, int* distant, int* small_distant, int* line_length, int* line_gap, int* kp, int* ki, int* kd, int* v){
     ...
     cvCreateTrackbar("ki", "pd", ki, 500 ,NULL);
	 cvCreateTrackbar("kd", "pd", kd, 1500 ,NULL);
	 cvCreateTrackbar("v", "pd", v, 200 ,NULL);
	 cvCreateTrackbar("ZAKOŃCZ", "pd", &koniec,1 ,NULL);
	 cvCreateTrackbar("eyeposition", "pd", &eye_position, 80 ,NULL);
	 cvCreateTrackbar("dthoriz", "pd", distant, 20 ,NULL);
	 cvCreateTrackbar("dtvert", "pd2", small_distant, 20 ,NULL);
	 //cvCreateTrackbar("prog_min", "pd", &(a->prog_min), 90 ,NULL);
	 cvCreateTrackbar("pprog", "pd2", &(a->plus_prog),80 ,NULL);
	 cvCreateTrackbar("rlewaprawa", "pd2", &(a->plus_roznica_lewaprawa),300 ,NULL);
	 cvCreateTrackbar("os", "pd2", &(a->odchylenie_standardowe),190 ,NULL);
	 cvCreateTrackbar("back", "pd2", line_gap, 50 ,NULL);
...
void echo_alert(int gpio, int lev, uint32_t tick)
{
	...
	if(gpio==ECHO && lev==0 && zbocze_narastajace==1)
	{
		...
		if(distance>0 && distance<400)
		{
			...
			if(distance_hcsr1<20)licznik_przeszkod++;
